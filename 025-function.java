...
class FunctionDemo
{
  ...
  // 不是必须放在主函数里面，只要在类里面就行了；主函数才是程序入口；JVM启动时会去找public static main函数，并且参数类型是字符串类型的数组
  ...
  default，默认的访问权限，也是可以省略的访问权限，它不仅能在设置了该权限的类中访问，也可以在同一包中的同一个类或子类中访问。
  protected，受保护的访问权限，它除了具有default的访问权限外，还可以在不同包中所继承的子类访问。
  public，公有的访问权限，也是最宽松的访问权限，不仅可以是同一个类或子类，还是同一个包中的同一个类或子类，又还是不同包中的同一个类或子类，都可以访问。
  */
